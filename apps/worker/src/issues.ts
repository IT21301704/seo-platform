...
export async function syncIssues(tx: Tx, args: SyncArgs): Promise<SyncResult> {
  ...
  const existingItems = (await tx.issueItem.findMany({ where: { projectId } })).filter((i) =>
    inScope(i.ruleId),
  );
  ...
  for (const { t, ids } of transitions.values()) {
    await tx.issueItem.updateMany({
      ...
      data: {
        ...
        lastCrawlId: crawlId,
        lastSeen: now,
        ...(t.regressed ? { regressedAt: now } : {}),
      },
    });
  }

  const passing = existingItems.filter(
    (i) => !failingKeys.has(i.stableKey) && i.auditTag !== "resolved",
  );
  for (const status of ["verified", "ignored"] as const) {
    ...
    if (ids.length)
      await tx.issueItem.updateMany({
        where: { id: { in: ids } },
        data: { status, auditTag: "resolved", lastCrawlId: crawlId },
      });
  }

  // Refresh open/total counts per issue.
  const grouped = await tx.issueItem.groupBy({
    by: ["issueId", "status", "auditTag"],
    where: { projectId },
    _count: { _all: true },
  });
