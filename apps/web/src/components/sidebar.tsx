    ...
    <>
      ...
      >
        ...
        <div className="flex flex-col gap-4">
          {groups.map((group) => (
            <div key={group.label} className="flex flex-col gap-0.5">
              <span className="px-3 pb-1 text-[11px] uppercase tracking-[0.08em] text-[#8D9197]">{group.label}</span>
                ...
                return (
                  <Link
                    ...
                    className={cn(
                      ...
                      active ? "bg-primary font-semibold text-white hover:text-white" : "font-medium text-[#C9CCD1] hover:bg-sidebar-card hover:text-white",
                    )}
                  >
                    {item.label}
                    {item.phase && <span className="text-[10px] font-semibold uppercase tracking-wide text-[#8D9197]">Soon</span>}
                  </Link>
                );
              })}
            </div>
        ...
        <div className="mt-auto flex items-center gap-2.5 border-t border-[#2C3038] px-2 pt-3">
          <div className="flex h-8 w-8 items-center justify-center rounded-full bg-[#3A3F48] text-[13px] font-semibold">{initials}</div>
