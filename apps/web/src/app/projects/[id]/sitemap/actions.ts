...
export async function runSitemapCheckAction(projectId: string, returnTo: "sitemap" | "urls"): Promise<void> {
  const { user, db } = await requireUser();
  assertCanEdit(user);
  await requireProject(db, projectId);
  const running = await db.sitemapCheck.findFirst({ where: { projectId, status: { in: ["queued", "running"] } } });
  if (!running) {
    ...
    await logAction(db, user, { action: "sitemap_check.start", entityType: "sitemap_check", entityId: check.id });
  }
...
export async function toggleAdded(projectId: string, urlId: string, added: boolean): Promise<void> {
  ...
  assertCanEdit(user);
  await requireProject(db, projectId);
  const row = await db.sitemapUrl.findUnique({ where: { id: urlId }, include: { check: { select: { projectId: true } } } });
  if (!row || row.check.projectId !== projectId || row.listType !== "manual_add") throw new Error("Not found");
  await db.sitemapUrl.update({ where: { id: urlId }, data: { added } });
  await logAction(db, user, { action: added ? "sitemap_url.mark_added" : "sitemap_url.unmark_added", entityType: "sitemap_url", entityId: urlId, after: { url: row.url } });
