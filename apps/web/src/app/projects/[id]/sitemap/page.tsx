...
export default async function SitemapCheckPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [latest, running, integrations] = await Promise.all([
    db.sitemapCheck.findFirst({
      where: { projectId: project.id, status: "completed" },
      orderBy: { createdAt: "desc" },
      include: { files: { orderBy: { url: "asc" } } },
    }),
    db.sitemapCheck.findFirst({
  ...
  const failed = await db.sitemapCheck.findFirst({
    where: {
  ...
  const states = latest
    ? await db.sitemapUrl.groupBy({
        by: ["gscState"],
        where: { checkId: latest.id, inSitemap: true, gscState: { not: null } },
        _count: { _all: true },
      })
    : [];
  ...
  const header = (
    <PageHeader
      eyebrow={
        latest ? (
          <>
            API check <Mono>{latest.id}</Mono> · {formatDateTime(latest.createdAt)} · ruleset v
      ...
      actions={
        <>
          {latest && (
            <a
              href={`/v1/sitemap-checks/${latest.id}`}
              target="_blank"
              rel="noreferrer"
      ...
      <PageBody>
        {running && (
          <p className="m-0 rounded-lg bg-primary-soft p-3 text-sm text-primary">
            A sitemap check is running. This page updates when it finishes.
          </p>
        )}
        {failed && (
          <p className="m-0 rounded-lg bg-crit-bg p-3 text-sm text-crit">
            The last check failed: {failed.error}
          </p>
        )}
        {!latest ? (
          <div className="grid grid-cols-1 gap-5 xl:grid-cols-[1fr_360px]">
            <EmptyState title="No sitemap check yet">
              <p className="m-0 text-muted">
                Run a check to validate your sitemaps, every listed URL, and (with Search Console)
            ...
            <div className="grid grid-cols-2 gap-5 md:grid-cols-3 xl:grid-cols-5">
              <Card className="flex flex-col gap-1 p-4">
                ...
                <span className="text-xs text-muted">
                  {applicable.filter((r) => r.status === "pass").length} of {applicable.length}{" "}
              ...
              <Card className="flex flex-col gap-1 p-4">
                ...
                <span className="text-[30px] font-bold leading-tight">
                  {formatNumber(latest.urlsInSitemaps)}
                </span>
                <span className="text-xs text-muted">
                  {latest.sitemapsCount - indexFiles}{" "}
                  {latest.sitemapsCount - indexFiles === 1 ? "file" : "files"}
              ...
              <Card className="flex flex-col gap-1 p-4">
                ...
                <span className="text-[30px] font-bold leading-tight">
                  {gsc ? formatNumber(gsc.indexed) : "—"}
                </span>
                <span className="text-xs text-muted">
                  {gsc ? `of ${formatNumber(gsc.inspected)} inspected` : "Connect Search Console"}
                </span>
              </Card>
              <Card className="flex flex-col gap-1 p-4">
                <span className="text-sm text-muted">Add manually</span>
                <span className="text-[30px] font-bold leading-tight">
                  {formatNumber(latest.manualUrls)}
                </span>
                <Link
                  href={`/projects/${project.id}/sitemap/urls`}
              ...
              <Card className="flex flex-col gap-1 p-4">
                ...
                <span className="text-[30px] font-bold leading-tight">
                  {formatNumber(latest.urlsToRemove)}
                </span>
                <Link
                  href={`/projects/${project.id}/sitemap/urls?tab=remove`}
                  className="text-[13px] font-semibold"
                >
                  Open URL list →
                </Link>
                    ...
                    <ul key={c} className="m-0 list-none p-0">
                      {col.map((r) => {
                        ...
                        return (
                          <li
                            key={r.ruleId}
                            className="flex items-center gap-3 border-b border-[#EDEDE8] py-2.5 text-sm"
                          >
                            <Mono className="w-[72px] shrink-0 text-muted">{r.ruleId}</Mono>
                            <Link
                              href={`/projects/${project.id}/issues/${r.ruleId}`}
                              className="flex-1 text-ink no-underline hover:text-primary"
                            >
                              {smpLabel(r)}
                            </Link>
                ...
                <Card className="flex flex-col gap-3 p-5">
                  ...
                  {!gsc ? (
                    <p className="m-0 text-sm text-muted">
                      Connect Search Console to see which sitemap URLs Google has indexed.
                    </p>
                  ) : (
                    <>
                      {[...GSC_STATES, ...otherStates.map((s) => s.gscState ?? "")].map((state) => {
                        ...
                        return (
                          <div key={state} className="flex flex-col gap-1 text-sm">
                            ...
                            <Bar
                              value={gsc.inspected ? (n / gsc.inspected) * 100 : 0}
                              tone={indexedState ? "pass" : "primary"}
                            />
                          </div>
                        );
                      })}
                      <p className="m-0 text-xs text-muted">
                        Inspected {formatNumber(gsc.inspected)} of{" "}
                        {formatNumber(latest.urlsInSitemaps)} · limit{" "}
                        {formatNumber(URL_INSPECTION_DAILY_LIMIT)} per day (
                        {formatNumber(quota.used)} used today) · data from{" "}
                        {formatDate(gsc.fetchedAt)}. URLs not inspected yet are not estimated.
                      </p>
                    </>
                  )}
                </Card>
                  ...
                  <Table>
                    ...
                    <tbody>
                      {latest.files.map((f) => (
                        <tr key={f.id}>
                          <Td>
                            <Mono title={`${f.generator} · found via ${f.discoveredVia}`}>
                              {pathOf(f.url)}
                            </Mono>
                          ...
                          <Td>
                            {f.kind === "sitemapindex" ? (
                              ...
                              formatNumber(f.urlCount)
                            ) : (
                              <Pill tone="crit">{f.status ?? "error"}</Pill>
                            )}
                          </Td>
                          <Td>
                            {f.gscLastDownloaded ? formatShortDate(f.gscLastDownloaded) : "—"}
                          </Td>
                        </tr>
                      ))}
                    </tbody>
        ...
        <p className="m-0 flex gap-2 rounded-[10px] border border-[#C9D3F5] bg-primary-soft p-4 text-sm">
          Sitemap and robots.txt fixes are high risk, so each one will need your approval (auto-fix
          arrives in Phase 3). After publishing, we re-check and resubmit the sitemap. Google
          decides what to index, so resubmitting can&apos;t guarantee indexing.
        </p>
      </PageBody>
    </>
  );
