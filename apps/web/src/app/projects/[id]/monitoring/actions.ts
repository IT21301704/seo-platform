...
export async function saveChannels(
  ...
  if (!email.success)
    return { ok: false, message: email.error.issues[0]?.message ?? "Invalid email" };
  ...
  if (slackUrl || existingSlack) {
    ...
    await db.alertChannel.upsert({
      ...
      update: {
        enabled: slackEnabled,
        target: label,
        ...(slackUrl ? { encryptedSecret: secret } : {}),
      },
    });
  ...
  await logAction(db, user, {
    action: "alerts.channels.update",
    entityType: "project",
    entityId: projectId,
    after: { emailEnabled, slackEnabled },
  });
...
export async function saveRules(
  ...
  if (!Number.isInteger(threshold) || threshold < 0 || threshold > 100)
    return { ok: false, message: "Threshold must be a whole number from 0 to 100" };
...
export async function saveSchedule(
  ...
  await db.project.update({
    where: { id: projectId },
    data: { ...parsed.data, nextCrawlAt: next },
  });
  await logAction(db, user, {
    action: "project.schedule.update",
    entityType: "project",
    entityId: projectId,
    after: parsed.data,
  });
