...
export default async function ConnectPage({
  ...
  const suggested =
    options.find((o) => o.id === `sc-domain:${host}` || o.id.includes(host))?.id ?? options[0]?.id;
        ...
        {error ? (
          ...
          <EmptyState title="No properties found">
            <p className="m-0 text-muted">
              This Google account has no{" "}
              {type === "gsc" ? "verified Search Console properties" : "GA4 properties"}.
            </p>
          </EmptyState>
        ) : (
          <Card className="max-w-xl p-6">
            <form action={select} className="flex flex-col gap-4">
              <label htmlFor="property" className="text-sm font-semibold">
                {type === "gsc"
                  ? "Search Console property for this website"
                  : "GA4 property for this website"}
              </label>
              <select
                id="property"
                name="property"
                defaultValue={
                  suggested
                    ? `${suggested}|${options.find((o) => o.id === suggested)?.name}`
                    : undefined
                }
                className="h-10 rounded-lg border border-[#CFCFC8] px-3 text-sm"
              >
                {options.map((o) => (
              ...
              <p className="m-0 text-xs text-muted">
                Read-only access. Data is saved as dated snapshots and refreshed daily at 02:00 site
                time.
              </p>
