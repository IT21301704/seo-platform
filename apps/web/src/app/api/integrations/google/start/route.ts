...
export async function GET(request: Request): Promise<Response> {
  ...
  if (user.role === "viewer" || !isGoogleType(type)) return new Response("Forbidden", { status: 403 });
  ...
  if (!client) {
    ...
    const data = {
      ...
      externalId: type === "gsc" ? `sc-domain:${new URL(project.rootUrl).hostname}` : "properties/000000000",
      externalName: type === "gsc" ? `${new URL(project.rootUrl).hostname} (demo data)` : "Demo GA4 property (demo data)",
      encryptedToken: null,
      scopes: [],
    ...
    await db.integration.upsert({
      ...
      create: { organizationId: user.organizationId, projectId: project.id, type, ...data } as Prisma.IntegrationUncheckedCreateInput,
      update: data,
    });
    await db.auditLog.create({ data: { organizationId: user.organizationId, actorId: user.id, action: `integration.${type}.connect_demo`, entityType: "project", entityId: project.id, source: "user" } });
    await enqueueGoogleSync({ projectId: project.id, organizationId: user.organizationId }, `connect-${Date.now()}`);
  ...
  jar.set(OAUTH_COOKIE, encryptSecret(JSON.stringify({ state, projectId: project.id, type, back, userId: user.id })), {
    httpOnly: true,
    sameSite: "lax",
