...
export const ONP_003 = defineRule(
  {
    ...
    explanation: {
      ...
      fix: [`Rewrite the title to ${TITLE_MIN}–${TITLE_MAX} characters, with the main topic first.`],
    },
  },
  (site) =>
    forPages(site, (p) => indexable(p) && Boolean(p.facts?.title), (p) => {
      const length = p.facts?.title?.length ?? 0;
      return length >= TITLE_MIN && length <= TITLE_MAX ? pass(p.url, { length }) : fail(p.url, { length });
    }),
);
