...
describe("Search Console sitemap rules", () => {
  ...
  it("SMP-003 passes when the sitemap is submitted and fails when it is not", async () => {
    expect(summary(await check(SMP_003, { gsc: { sitemaps: [sitemap] } }))).toEqual([
      "pass /sitemap.xml",
    ]);
  ...
  it("SMP-013 fails when Search Console reports errors or warnings", async () => {
    expect(summary(await check(SMP_013, { gsc: { sitemaps: [sitemap] } }))).toEqual([
      "pass /sitemap.xml",
    ]);
    expect(
      summary(await check(SMP_013, { gsc: { sitemaps: [{ ...sitemap, warnings: 3 }] } })),
    ...
    const outcomes = await check(SMP_014, {
      gsc: {
        inspections: [
          ...
          inspection("/about/", "NEUTRAL", "Crawled - currently not indexed"),
        ],
      },
    });
    expect(summary(outcomes)).toEqual(["pass /", "fail /about/"]);
    const partial = await check(SMP_014, {
      gsc: { inspections: [inspection("/", "PASS", "Submitted and indexed")] },
    });
