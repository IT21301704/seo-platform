...
export const SMP_007 = defineRule(
  {
    ...
    passCondition: "Passes when every sitemap URL we fetched returns 200 directly (no redirect, no 4xx/5xx).",
    appliesTo: "url",
    autoFixable: true,
    riskLevel: "high",
    effort: 1,
    explanation: {
      ...
      fix: ["Remove broken URLs from the sitemap, or replace redirected ones with their final URL, through your CMS settings."],
    },
  },
  (site) =>
    [...site.sitemapEntries.keys()].flatMap((url) => {
      ...
      if (chain.length > 0) return [fail(url, { problem: "redirects", to: page.record.finalUrl, status: chain[0]?.status ?? null })];
