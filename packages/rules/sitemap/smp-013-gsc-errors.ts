...
export const SMP_013 = defineRule(
  {
    ...
    explanation: {
      ...
      fix: ["Open the sitemap in Search Console to see the error details.", "Fix the cause in the CMS or plugin that generates the sitemap, then resubmit."],
    },
  },
  (site) =>
    (site.gsc?.sitemaps ?? []).map((s) => {
      const evidence = { errors: s.errors, warnings: s.warnings, dataDate: site.gsc?.dataDate ?? null };
