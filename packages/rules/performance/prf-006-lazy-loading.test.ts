...
const withImages = (body: string) => ({
  pages: { ...defaultPages(), "/about/": page("/about/", { body }) },
});

describe("PRF-006 lazy loading", () => {
  it("passes for an eager hero and lazy later images", async () => {
    const body =
      '<img src="/hero-mug.svg" alt="Hero" fetchpriority="high"><img src="/detail-shot.svg" alt="Detail" loading="lazy">';
  ...
  it("fails for a lazy hero and an eager later image", async () => {
    const body =
      '<img src="/hero-mug.svg" alt="Hero" loading="lazy"><img src="/detail-shot.svg" alt="Detail">';
