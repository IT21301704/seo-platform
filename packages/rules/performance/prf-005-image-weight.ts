...
export const PRF_005 = defineRule(
  {
    ...
    explanation: {
      ...
      fix: [
        ...
        "Convert photos to WebP or AVIF (many CMS plugins do this automatically).",
      ],
    },
  },
  (site) =>
    forPages(site, indexable, (p) => {
      const checked = [
        ...new Set(
          (p.facts?.images ?? []).map((i) => i.url).filter((u): u is string => u !== null),
        ),
      ]
      ...
      const heavy = checked
        ...
        .filter(
          (r) =>
            r.bodySize > MAX_IMAGE_BYTES ||
            (LEGACY.test(r.contentType ?? "") && r.bodySize > MAX_LEGACY_IMAGE_BYTES),
        )
        .map((r) => ({ url: r.url, bytes: r.bodySize, type: r.contentType }));
      return heavy.length
        ? fail(p.url, { images: heavy })
        : pass(p.url, { images: checked.length });
    }),
);
