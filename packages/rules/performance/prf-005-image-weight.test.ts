...
const pages = {
  ...defaultPages(),
  "/about/": page("/about/", { body: '<img src="/images/studio-photo.png" alt="Studio">' }),
};

describe("PRF-005 image weight", () => {
  it("passes for a small PNG", async () => {
    const outcomes = await check(PRF_005, {
      pages,
      files: { "/images/studio-photo.png": Buffer.alloc(20 * 1024) },
    });
  ...
  it("fails for a 150 KB PNG", async () => {
    const outcomes = await check(PRF_005, {
      pages,
      files: { "/images/studio-photo.png": Buffer.alloc(150 * 1024) },
    });
