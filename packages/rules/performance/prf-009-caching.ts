...
export const PRF_009 = defineRule(
  {
    ...
    passCondition: "Passes when the page response has Cache-Control (not no-store), ETag or Last-Modified.",
    appliesTo: "url",
    autoFixable: false,
    riskLevel: "low",
    effort: 1,
    explanation: {
      ...
      fix: ["Enable page caching in your host or caching plugin, or set Cache-Control at your CDN."],
    },
  },
  (site) =>
    forPages(site, indexable, (p) => {
      ...
      const ok = (cacheControl !== "" && !/no-store/i.test(cacheControl)) || Boolean(h["etag"]) || Boolean(h["last-modified"]);
