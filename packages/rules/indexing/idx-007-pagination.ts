...
export const IDX_007 = defineRule(
  {
    ...
    passCondition: "Passes when page 2, 3, ... of a paginated list has a self-referencing canonical (not page 1).",
    appliesTo: "both",
    autoFixable: false,
    riskLevel: "medium",
    effort: 2,
    explanation: {
      ...
      fix: ["Give each paginated page a canonical to itself.", "Keep normal <a href> links between the pages."],
    },
  },
  (site) =>
    forPages(site, (p) => p.isHtml200 && (pageNumber(p.url) ?? 1) > 1, (p) => {
