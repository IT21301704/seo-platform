...
export const AI_005 = defineRule(
  {
    ...
    passCondition:
      "Passes when no important page uses nosnippet or max-snippet:0 in meta robots or X-Robots-Tag.",
    appliesTo: "both",
    autoFixable: true,
    riskLevel: "high",
    effort: 1,
    explanation: {
      ...
      fix: [
        "Remove nosnippet / max-snippet:0 from these pages.",
        "To hide one section only, use data-nosnippet on that element.",
      ],
    },
  },
  (site) =>
    site.importantUrls.flatMap((url) => {
      ...
      const blocking = page.facts.robotsDirectives.filter(
        (d) => d === "nosnippet" || d === "max-snippet:0",
      );
