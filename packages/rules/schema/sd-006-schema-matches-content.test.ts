...
const faq = (answer: string) => ({
  "@type": "FAQPage",
  mainEntity: [{ "@type": "Question", name: "Do you ship?", acceptedAnswer: { "@type": "Answer", text: answer } }],
});
const body = "<h2>Do you ship?</h2><p>Yes, within two to four working days.</p><p>Price: LKR 4,500</p>";
...
describe("SD-006 schema matches content", () => {
  it("passes when FAQ and price match the page", async () => {
    ...
    const pages = {
      ...defaultPages(),
      "/about/": page("/about/", { body, jsonLd: [faq("Yes, within two to four working days."), product] }),
    };
  ...
  it("fails when the FAQ answer differs from the visible text", async () => {
    const pages = { ...defaultPages(), "/about/": page("/about/", { body, jsonLd: [faq("Always next day.")] }) };
