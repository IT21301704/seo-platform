...
export const SD_005 = defineRule(
  {
    ...
    explanation: {
      ...
      fix: ["Use your SEO plugin's breadcrumb feature so the markup matches the visible breadcrumbs."],
    },
  },
  (site) =>
    forPages(site, indexable, (p) => {
      ...
      for (const bl of lists) {
        ...
        items.forEach((item, i) => {
          if (Number(item["position"]) !== i + 1) problems.push(`item ${i + 1}: position should be ${i + 1}`);
          ...
          if (i < items.length - 1 && !(url && /^https?:\/\//.test(url))) problems.push(`item ${i + 1}: missing absolute URL`);
        });
      }
