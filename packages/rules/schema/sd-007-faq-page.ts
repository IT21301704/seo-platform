...
export const SD_007 = defineRule(
  {
    ...
    passCondition: "Passes when every FAQPage has at least one Question, each with a name and an acceptedAnswer text.",
    appliesTo: "both",
    autoFixable: true,
    riskLevel: "high",
    effort: 1,
    explanation: {
      ...
      fix: ["Give each Question a name (the question) and an acceptedAnswer with the full answer text."],
    },
  },
  (site) =>
    forPages(site, indexable, (p) => {
      ...
      for (const faq of faqs) {
        ...
        questions.forEach((q, i) => {
          ...
          if (!str(obj(q["acceptedAnswer"])?.["text"])) problems.push(`question ${i + 1}: missing answer text`);
        });
      }
