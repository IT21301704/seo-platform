...
const client = { clientId: "cid", clientSecret: "secret", redirectUri: "https://app.example/api/integrations/google/callback" };
  ...
  it("exchanges a code and refreshes an expired token", async () => {
    const http = new FakeHttp((req) =>
      req.form?.["grant_type"] === "authorization_code"
        ? { status: 200, body: { access_token: "a1", refresh_token: "r1", expires_in: 3600, scope: "s" } }
        : { status: 200, body: { access_token: "a2", expires_in: 3600, scope: "s" } },
    );
    const tokens = await exchangeCode(http, client, "code", 1_000);
    expect(tokens).toEqual({ refreshToken: "r1", accessToken: "a1", expiresAt: 3_601_000, scope: "s" });
...
describe("Search Console client", () => {
  const http = new FakeHttp((req) => {
    if (req.url.endsWith("/searchAnalytics/query")) {
      return { status: 200, body: { rows: [{ keys: ["https://a.com/x/"], clicks: 5, impressions: 100, ctr: 0.05, position: 4.2 }] } };
    }
    if (req.url.endsWith("/sitemaps")) {
      return { status: 200, body: { sitemap: [{ path: "https://a.com/sitemap.xml", errors: "2", warnings: "1", contents: [{ submitted: "10", indexed: "7" }] }] } };
    }
    if (req.url.includes("urlInspection")) {
      return { status: 200, body: { inspectionResult: { indexStatusResult: { verdict: "NEUTRAL", coverageState: "Crawled - currently not indexed" } } } };
    }
  ...
  it("parses search analytics, sitemaps and inspections with a bearer token", async () => {
    expect(await gsc.searchAnalytics("sc-domain:a.com", { startDate: "2026-08-26", endDate: "2026-09-22" })).toEqual([
      { page: "https://a.com/x/", clicks: 5, impressions: 100, ctr: 0.05, position: 4.2 },
    ]);
    expect((await gsc.listSitemaps("sc-domain:a.com"))[0]).toMatchObject({ errors: 2, warnings: 1, submitted: 10, indexed: 7 });
...
describe("GA4 and CrUX", () => {
  it("reads sessions per page", async () => {
    const http = new FakeHttp(() => ({
      status: 200,
      body: { rows: [{ dimensionValues: [{ value: "/a" }], metricValues: [{ value: "10" }] }, { dimensionValues: [{ value: "/b" }], metricValues: [{ value: "30" }] }] },
    }));
    expect(await new GoogleGa4Api(http, async () => "t").sessionsByPage("properties/1", { startDate: "a", endDate: "b" })).toEqual([
      { path: "/b", sessions: 30 },
      { path: "/a", sessions: 10 },
    ]);
  });
  ...
  it("bands CrUX p75 values and treats 404 as no data", async () => {
    const http = new FakeHttp(() => ({
      status: 200,
      body: { record: { metrics: { largest_contentful_paint: { percentiles: { p75: 3100 } }, cumulative_layout_shift: { percentiles: { p75: "0.05" } } } } },
    }));
    ...
    expect(await queryCruxOrigin(new FakeHttp(() => ({ status: 404, body: {} })), "k", "https://a.com")).toBeNull();
  });
});
...
describe("URL Inspection quota", () => {
  ...
  it("inspects never-seen URLs first, then stale ones, by traffic, within the remaining quota", () => {
    const plan = planInspections(
      [
        { url: "https://a.com/fresh", traffic: 999, lastInspectedAt: new Date(now.getTime() - day) },
        { url: "https://a.com/stale", traffic: 500, lastInspectedAt: new Date(now.getTime() - 10 * day) },
    ...
    expect(plan).toEqual(["https://a.com/new-high", "https://a.com/new-low", "https://a.com/stale"]);
  });

  it("never exceeds the remaining quota", () => {
    const many = Array.from({ length: 5000 }, (_, i) => ({ url: `https://a.com/${i}`, traffic: i, lastInspectedAt: null }));
  ...
  it("spaces calls to 600 per minute", async () => {
    const sleeps: number[] = [];
    const limiter = new RateLimiter(600, async (ms) => void sleeps.push(ms), () => 0);
...
describe("demo provider", () => {
  it("is deterministic", async () => {
    const make = () => new DemoGscApi("https://example-store.com", async () => ["https://example-store.com/a/", "https://example-store.com/b/"], async () => [], () => new Date(0));
    expect(await make().searchAnalytics()).toEqual(await make().searchAnalytics());
    expect(await make().inspect("s", "https://example-store.com/blog/old-post/")).toMatchObject({ verdict: "FAIL" });
  });
});
